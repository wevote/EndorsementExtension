...
function getHighlightsListsFromApiServer (locationHref, voterDeviceId, doReHighlight, sendResponse, showVoterGuideHighlights, showCandidateOptionsHighlights) {
  ...
  if (showVoterGuideHighlights && showCandidateOptionsHighlights) {
    ...
    fetch(voterGuidePossibilityHighlightsRetrieve).then((resp) => resp.json()).then((voterGuidePossibilityHighlightsRetrieveResponse) => {
      ...
      fetch(ballotItemHighlightsRetrieve).then((resp) => resp.json()).then((ballotItemHighlightsRetrieveResponse) => {
        ...
        processHighlightsRetrieve(ballotItemHighlightsRetrieveResponse, voterGuidePossibilityHighlightsRetrieveResponse,
    ...
    // Get the entries already found from page scan
    console.log('background showVoterGuideHighlights', voterGuidePossibilityHighlightsRetrieve);
    fetch(voterGuidePossibilityHighlightsRetrieve).then((resp) => resp.json()).then((voterGuidePossibilityHighlightsRetrieveResponse) => {
...
function getVoterSignInInfo (sendResponse) {
  let data = {};
  chrome.storage.local.get(['voterDeviceId'], (store) => {
    const { voterDeviceId } = store;
    console.log('getVoterSignInInfo key Value currently is ', voterDeviceId);
